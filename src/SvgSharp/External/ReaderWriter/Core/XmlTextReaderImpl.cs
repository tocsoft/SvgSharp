...
namespace SharpSvg.System.Xml
{
    internal partial class XmlTextReaderImpl : XmlReader, IXmlLineInfo, IXmlNamespaceResolver
    {
        ...
        private XmlTextReaderImpl(XmlResolver resolver, XmlReaderSettings settings, XmlParserContext context)
        {
            ...
            _dtdProcessing = (DtdProcessing)Enum.Parse(typeof(DtdProcessing), settings.DtdProcessing.ToString()); ;
            ...
            : this(null, settings, context)
        {
            ...
            if (context != null)
            {
                if (context.BaseURI != null && context.BaseURI.Length > 0 &&
                    !UriEqual(baseUri, baseUriStr, context.BaseURI, null))
                {
        ...
        public override XmlReaderSettings Settings
        {
            get
            {
                ...
                settings.DtdProcessing = (DtdProcessing)Enum.Parse(typeof(DtdProcessing), _dtdProcessing.ToString());
                settings.MaxCharactersInDocument = _maxCharactersInDocument;
                settings.MaxCharactersFromEntities = _maxCharactersFromEntities;

                //settings.ReadOnly = true; //not supported
                return settings;
            }
        }
        ...
        public override int ReadContentAsBase64(byte[] buffer, int index, int count)
        {
            ...
            else
            {
                ...
                if (!XmlReaderExtensions.CanReadContentAs(_curNode.type))
                {
        ...
        {
            // throw on elements
            if (!XmlReaderExtensions.HasValueInternal(_curNode.type))
            {
        ...
        private void InitStreamInput(Uri baseUri, string baseUriStr, Stream stream, byte[] bytes, int byteCount, Encoding encoding)
        {
            ...
            else
            {
                ...
                {
                    bufferSize = XmlReaderExtensions.AsyncBufferSize;
                }
                else
                {
                    bufferSize = XmlReaderExtensions.CalcBufferSize(stream);
                }
        ...
        private void InitTextReaderInput(string baseUriStr, Uri baseUri, TextReader input)
        {
            ...
            if (_ps.chars == null)
            {
                if (_laterInitParam != null && _laterInitParam.useAsync)
                {
                    _ps.chars = new char[XmlReaderExtensions.AsyncBufferSize + 1];
                }
                else
                {
                    _ps.chars = new char[XmlReaderExtensions.DefaultBufferSize + 1];
                }
            }
        ...
        private void ProcessDtdFromParserContext(XmlParserContext context)
        {
            ...
            switch (_dtdProcessing)
            {
                ...
                case (DtdProcessing)2://cant use DtdProcessing.Parse - see https://github.com/dotnet/corefx/issues/4376
        ...
        private EntityType HandleGeneralEntityReference(string name, bool isInAttributeValue, bool pushFakeEntityIfNullResolver, int entityStartLinePos)
        {
            ...
            if (_dtdInfo == null && _fragmentParserContext != null && _fragmentParserContext.HasDtdInfo && _dtdProcessing == (DtdProcessing)2) //cant use DtdProcessing.Parse - see https://github.com/dotnet/corefx/issues/4376
            {
                ParseDtdFromParserContext();
            }
        ...
        private bool ParseDoctypeDecl()
        {
            ...
            if (_dtdProcessing == (DtdProcessing)2) //cant use DtdProcessing.Parse - see https://github.com/dotnet/corefx/issues/4376
            {
                _curNode.SetLineInfo(_ps.LineNo, _ps.LinePos);
        ...
        private bool InitReadContentAsBinary()
        {
            ...
            if (!XmlReaderExtensions.IsTextualNode(_curNode.type))
            {
